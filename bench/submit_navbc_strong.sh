...
#SBATCH -J navstr
#SBATCH --array=1-6
#SBATCH -o /n/netscratch/lichtman_lab/Lab/fumingyang/trackmaze/bench/navstr_%A_%a.out
...
CFG=$(sed -n "${SLURM_ARRAY_TASK_ID}p" configs_navbc_strong.txt)
ARCH=$(echo $CFG | awk '{print $1}'); SEED=$(echo $CFG | awk '{print $2}')
echo "task $SLURM_ARRAY_TASK_ID: arch=$ARCH seed=$SEED (STRONG: more data+epochs)"
